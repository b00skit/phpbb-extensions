...
namespace booskit\privatemessageas\event;
...
class listener implements EventSubscriberInterface
{
	...
	{
		return [
			'core.ucp_pm_compose_template' => 'inject_privatemessageas_dropdown',
			'core.submit_pm_after' => 'save_privatemessageas_selection',
			'core.ucp_pm_view_message' => 'modify_pm_display',
		];
	}

	public function inject_privatemessageas_dropdown($event)
	{
		$this->user->add_lang_ext('booskit/privatemessageas', 'privatemessageas');
		...
		$this->template->assign_block_vars('privatemessageas_options', [
			'VALUE' => 0,
			'NAME' => $this->user->lang('PRIVATEMESSAGEAS_YOURSELF'),
			'COLOR' => '',
			'RANK' => '',
		]);
		...
		$template_ary['S_PRIVATEMESSAGEAS_AVAILABLE'] = true;
		$event['template_ary'] = $template_ary;
	}

	public function save_privatemessageas_selection($event)
	{
		$privatemessageas_altchar_id = $this->request->variable('privatemessageas_altchar_id', 0);
		if ($privatemessageas_altchar_id <= 0)
		{
			return;
		}
		...
		$altchar = $this->get_altchar_by_id($privatemessageas_altchar_id, $this->user->data['user_id']);
		if (empty($altchar))
		{
			return;
		}

		$sql = 'INSERT INTO ' . $this->table_prefix . 'booskit_privatemessageas ' .
			$this->db->sql_build_array('INSERT', [
				'msg_id' => $pm_id,
				'user_id' => (int) $this->user->data['user_id'],
				'altchar_id' => (int) $privatemessageas_altchar_id,
			]);
	...
	public function modify_pm_display($event)
	{
		...
		$privatemessageas_data = $this->get_privatemessageas_data($msg_id);
		if (empty($privatemessageas_data))
		{
			return;
		}

		$altchar_id = (int) $privatemessageas_data['altchar_id'];
		$check_user_id = isset($privatemessageas_data['user_id']) ? (int) $privatemessageas_data['user_id'] : $author_id;
		...
		$msg_data['MESSAGE_AUTHOR_FULL'] = $this->build_privatemessageas_author_full($altchar['name'], $display_color, $real_username, $u_message_author);
	...
	private function get_privatemessageas_data($msg_id)
	{
		$sql = 'SELECT * FROM ' . $this->table_prefix . 'booskit_privatemessageas WHERE msg_id = ' . (int) $msg_id;
		$result = $this->db->sql_query($sql);
		$row = $this->db->sql_fetchrow($result);
		$this->db->sql_freeresult($result);
		return $row;
	...
	private function build_privatemessageas_author_full($alt_name, $alt_color, $real_username, $u_profile)
	{
		...
		$show_original = isset($this->config['privatemessageas_show_original']) ? (bool) $this->config['privatemessageas_show_original'] : true;

		$real_span = (!empty($real_username) && $show_original)
			? '<span class="privatemessageas-realname" style="color: #000;"> (' . $real_username . ')</span>'
			: '';
