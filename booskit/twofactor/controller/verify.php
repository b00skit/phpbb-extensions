...
class verify
{
    ...
    public function handle()
    {
        ...
        if (!$this->manager->is_user_2fa_enabled($user_id)) {
            ...
            // Not enabled and not enforced -> redirect to board index
        ...
        if ($this->request->is_set_post('submit')) {
            if (!check_form_key('booskit_2fa_verify')) {
                ...
                $auth_mode = $this->request->variable('auth_mode', 'totp'); // 'totp' or 'backup'
                ...
                if ($auth_mode === 'backup' || (!empty($backup_code) && empty($totp_code))) {
                    ...
                    if ($this->backup_codes->verify_and_consume_code($user_id, $backup_code)) {
                        ...
                        $target = $this->get_safe_redirect_url();
        ...
        $this->template->assign_vars([
            ...
            'REDIRECT'              => $redirect,
            'MODULE'                => $module,
            'U_ACTION'              => $this->helper->route('booskit_twofactor_verify'),
    ...
    protected function get_safe_redirect_url()
    {
